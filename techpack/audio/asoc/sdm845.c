...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (get_hw_version_platform() == HARDWARE_PLATFORM_POLARIS || get_hw_version_platform() ==
		HARDWARE_PLATFORM_DIPPER || get_hw_version_platform() == HARDWARE_PLATFORM_DIPPERN
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "tavil_codec")) {
		...
		if (of_property_read_bool(dev->of_node,
			...
			if ((get_hw_version_platform() == HARDWARE_PLATFORM_DIPPER) ||
